...
for ticket in tickets:
    ...
    print(f"[{entrada['ticket_id']}]", end=" ", flush=True)
    ...
    try:
        ...
        print(
            f"{status} cat={resultado.get('category', '?'):10} "
            f"prio={resultado.get('priority', '?'):8} "
            f"rota={rota} [{tempo_ms}ms]"
        )

    except Exception as e:
        tempo_ms = round((time.time() - inicio) * 1000)
        erros += 1
        erros_detalhe.append({"ticket_id": entrada["ticket_id"], "erro": str(e)})
        print(f"✗ ERRO: {e}")
